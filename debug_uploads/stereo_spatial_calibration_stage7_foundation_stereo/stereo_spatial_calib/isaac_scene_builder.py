...
class IsaacStereoSceneBuilder:
    ...
    def __init__(self, config):
        ...
        self.world = None
        self.semantic_label_set_success = False
    ...
    def launch(self):
        try:
            ...
            self.simulation_app = SimulationApp({"headless": True, "enable_cameras": True})
    ...
    def _build_stage_with_world(self, object_type, object_position):
        ...
        try:
            add_labels(obj.prim, labels=[f"target_{object_type}", object_type, "target_object"], instance_name="class")
            self.semantic_label_set_success = True
        except Exception:
            self.semantic_label_set_success = False
    ...
    @staticmethod
    def transform_to_world_camera_pose(T_world_cam):
        """Return an Isaac Camera pose using Isaac's ``camera_axes="world"``.

        Metadata uses OpenCV optical axes (x right, y down, z forward). Isaac
        Camera's world camera axes are x forward, z up. The equivalent camera
        basis is therefore x=forward, y=left, z=up.
        """
        ...
        forward = T[:3, 2]
        up = -down
        R_world_axes = np.stack([forward, -right, up], axis=1).astype(np.float32)
        return T[:3, 3].copy(), rotation_matrix_to_quat_wxyz(R_world_axes)
