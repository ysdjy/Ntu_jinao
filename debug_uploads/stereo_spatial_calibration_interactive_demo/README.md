...
Point-cloud reference convention is fixed:

- FoundationStereo consumes `left_rgb` and `right_rgb`.
- FoundationStereo disparity/depth is interpreted in the left reference view.
- The primary point cloud is backprojected from left-image pixels and predicted left-view depth.
- The point cloud starts in the left camera frame.
- `T_world_left_camera` / `T_world_left_cam` maps left-camera points into world.
- `right_rgb` is required for stereo matching, but it does not directly generate the primary point cloud.
- Reports include `pointcloud_reference_view="left"`, `T_world_reference_camera`, and `center_pred_left_camera_m`.
- Older `center_pred_camera_m` fields are kept for compatibility and mean `center_pred_left_camera_m`.

Stereo rig convention:

- The rig parent is `/World/StereoRig`.
- Left camera is `/World/StereoRig/left_camera`.
- Right camera is `/World/StereoRig/right_camera`.
- Move `/World/StereoRig` as the parent when using the GUI; do not manually decouple the child cameras.
- If the child camera baseline is broken, use the repair helper to restore left local pose to identity and right local pose to `[baseline_m, 0, 0]`.

Interactive smoke test:

```bash
ENABLE_CAMERAS=1 TERM=xterm ./isaaclab.sh -p \
  source/standalone/stereo_spatial_calibration/scripts/launch_interactive_demo.py \
  --headless_smoke_test \
  --baseline 0.10 \
  --resolution 640 480
```

Latest headless smoke result:
...
runtime_ms=25110.678
depth_rmse_m=0.005633
error_l2_mm=64.350
```

The larger interactive smoke error indicates the demo path still needs a perception/calibration pass before robot execution. The smoke test is meant to validate interactive capture, FoundationStereo inference, report generation, and coordinate plumbing, not to certify execution-ready accuracy.
...
On this remote session, GUI startup entered the Isaac application loop and was stopped with a short `timeout` during validation. For actual manual interaction, run it in a desktop Isaac Sim session such as local display, NoMachine, or a configured X11 environment. The UI is button-triggered single-shot inference; it does not run FoundationStereo every frame.

Current FoundationStereo throughput remains approximately one single-shot inference every 20-25 seconds on this setup. The UI is therefore intended for manual inspection and calibration, not real-time closed-loop control.
