...
from .native_segmentation import attach_segmentation_annotators, select_mask_from_frame
...
def _step_builder_render(builder, count=1):
    for _ in range(max(1, int(count))):
        stepped = False
        if getattr(builder, "world", None) is not None:
            try:
                builder.world.step(render=True)
                stepped = True
            except Exception:
                stepped = False
        if not stepped:
...
def _camera_frame(camera):
    try:
        return camera.get_current_frame(clone=True)
    except TypeError:
...
def _try_camera_mask(camera, shape, mask_source="auto", target_names=()):
    frame = _camera_frame(camera)
    selected = select_mask_from_frame(frame, shape, mask_source=mask_source, target_names=target_names)
    ...
    for add_name, key in (
        ...
        try:
            ...
            if arr.shape[:2] == shape and (arr.astype(np.int64) > 0).any():
                selected.mask = arr.astype(np.int64) > 0
                selected.mask_source = key
                selected.segmentation_available = True
                return selected
        except Exception:
            continue
    return selected
...
def _base_metadata(sample_id, object_type, object_name, center_world, width, height, baseline_m, T_world_left, T_world_right):
    ...
    return {
        ...
        "T_world_left_camera": np.asarray(T_world_left, dtype=np.float32).tolist(),
        "T_world_right_camera": np.asarray(T_world_right, dtype=np.float32).tolist(),
        "pointcloud_reference_view": "left",
        "T_world_reference_camera": np.asarray(T_world_left, dtype=np.float32).tolist(),
    ...
    metadata.update(
        {
            ...
            "rig_prim_path": "/World/StereoRig",
            "left_camera_prim_path": "/World/StereoRig/left_camera",
            "right_camera_prim_path": "/World/StereoRig/right_camera",
            "baseline_expected_m": float(baseline_m),
            "baseline_actual_m": float(np.linalg.norm(np.asarray(T_world_right)[:3, 3] - np.asarray(T_world_left)[:3, 3])),
            "baseline_error_m": float(
                np.linalg.norm(np.asarray(T_world_right)[:3, 3] - np.asarray(T_world_left)[:3, 3]) - float(baseline_m)
            ),
            "stereo_rig_valid": bool(
                abs(np.linalg.norm(np.asarray(T_world_right)[:3, 3] - np.asarray(T_world_left)[:3, 3]) - float(baseline_m))
                < 1e-4
            ),
            "segmentation_available": bool(segmentation_available),
            "semantic_label_set_success": bool(semantic_label_set_success),
            "target_instance_id": target_instance_id,
            "native_frame_summary": native_frame_summary or {},
...
def capture_with_camera_class(builder, sample_id, object_type, center_world, width, height, baseline_m, mask_source="auto"):
    ...
    if UsdGeom is not None:
        try:
            ...
            UsdGeom.Xform.Define(stage, "/World/StereoRig")
        except Exception:
            pass
    T_world_left, T_world_right = builder.camera_pose()
    left_pos, left_quat = builder.transform_to_world_camera_pose(T_world_left)
    right_pos, right_quat = builder.transform_to_world_camera_pose(T_world_right)

    left_cam = Camera(
        "/World/StereoRig/left_camera",
        name="left_camera",
        frequency=60,
    ...
    for cam, pos, quat in ((left_cam, left_pos, left_quat), (right_cam, right_pos, right_quat)):
        if hasattr(cam, "set_world_pose"):
            cam.set_world_pose(position=pos, orientation=quat, camera_axes="world")
        cam.initialize()
        if hasattr(cam, "set_resolution"):
            cam.set_resolution((int(width), int(height)))
        if hasattr(cam, "set_clipping_range"):
            cam.set_clipping_range(0.05, 100.0)
        ...
        if cam is left_cam:
            attach_segmentation_annotators(cam, mask_source=mask_source)
    ...
    try:
        ...
        _step_builder_render(builder, count=10)
    for _ in range(80):
        _step_builder_render(builder, count=1)
        try:
    ...
    selected = _try_camera_mask(
        left_cam,
        depth.shape,
        mask_source=mask_source,
        target_names=(f"target_{object_type}", object_type, "target_object"),
    )
    mask = selected.mask
    seg_available = mask is not None and selected.segmentation_available
    selected_source = selected.mask_source or "projected_bbox_fallback"
    if mask is None:
        ...
        selected_source = "projected_bbox_fallback"
    return _finish_result(
        ...
        semantic_label_set_success=bool(getattr(builder, "semantic_label_set_success", False)),
        target_instance_id=selected.target_instance_id,
        native_frame_summary=selected.frame_summary,
    )
    ...
    for method in methods:
        try:
            if method == "camera_class":
                result = capture_with_camera_class(
                    builder, sample_id, object_type, center_world, width, height, baseline_m, mask_source=mask_source
                )
