...
def depth_metrics(pred_depth, gt_depth, mask=None):
    ...
    if not valid.any():
        return {
            "depth_mae_m": np.nan,
            "depth_rmse_m": np.nan,
            "depth_error_median_m": np.nan,
            "depth_error_p90_m": np.nan,
    ...
    return {
        ...
        "depth_error_median_m": float(np.median(abs_diff)),
        "depth_error_p90_m": float(np.percentile(abs_diff, 90)),
        "depth_error_p95_m": float(np.percentile(abs_diff, 95)),
...
def evaluate_sample(sample_dir, prediction_dir, method="pointcloud_median", erode_kernel_size=3):
    ...
    row = {
        ...
        "method": method,
        "pointcloud_reference_view": metadata.get("pointcloud_reference_view", "left"),
        "T_world_reference_camera": metadata.get("T_world_reference_camera", metadata.get("T_world_left_cam")),
        "center_camera_raw_m": center_camera_raw.tolist(),
        "center_left_camera_raw_m": center_camera_raw.tolist(),
        "native_surface_center_correction_z_m": float(center_correction_z_m),
        "center_camera_m": center_camera.tolist(),
        "center_left_camera_m": center_camera.tolist(),
        "center_world_m": center_world.tolist(),
        "center_base_m": center_base.tolist(),
        "center_pred_left_camera_m": center_camera.tolist(),
        ...
        "checkpoint_ready": bool(runtime.get("checkpoint_ready", runtime.get("checkpoint_available", False))),
